...
由 Shigma 设计的元框架 **[Cordis](https://cordis.moe/zh-CN/guide/features.html)**（其学术阐述见《*A Programming Paradigm for Spatiotemporal Composability*》）在软件架构层面具有极高的理论纯度。
...
```mermaid
graph TD
    subgraph S1 ["AstrBot 核心架构突破 (DX 优势)"]
        direction LR
    ...
    subgraph S2 ["潜在生命周期暗礁 (架构隐患)"]
        direction LR
        B1["terminate() 约定式清理<br/>(缺乏框架强制追踪)"]
        B2["全局扁平 Context<br/>(多 Agent / 租户状态踩踏)"]
        B3["单步 JSON Tool Calling<br/>(高 Token / 网络往返)"]
    end

    S1 ==>|"在极度追求极简 DX 的同时，下沉隐藏了不可逆隐患"| S2
```
...
1. **Cordis 核心设计理论与元框架**
   ...
   - Cordis 官方设计文档与特性指南: [https://cordis.moe/zh-CN/guide/features.html](https://cordis.moe/zh-CN/guide/features.html)
2. **Koishi 跨平台机器人系统**
