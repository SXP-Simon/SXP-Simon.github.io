...
支持官方单曲、专辑、歌单以及**社区/播客声音（电台节目）**嵌入，**构建时自动抓取曲名与作者，无需手动填写**：

#### 示例 1：官方单曲（只需填 ID，自动识别曲名与歌手）
<NetEaseMusic id="32098307" />

#### 示例 2：社区声音 / 播客（直接贴网易云分享链接，自动解析类型与标题）
<NetEaseMusic url="https://y.music.163.com/m/program?id=2488308450" />

```mdx
import NetEaseMusic from "@/components/mdx/NetEaseMusic.astro";

<!-- 1. 极简零配置：只需传入单曲 ID，组件自动拉取歌名与歌手 -->
<NetEaseMusic id="32098307" />

<!-- 2. 直接粘贴网易云分享链接（单曲/播客声音/歌单均可自动解析） -->
<NetEaseMusic url="https://y.music.163.com/m/program?id=2488308450" />

<!-- 3. 完整歌单卡片（450px 大封面模式） -->
<NetEaseMusic type="playlist" id="3778678" />

<!-- 4. 也支持手动覆盖自定义标题与艺术家 -->
<NetEaseMusic id="32098307" title="Wings of Courage" artist="川田まみ" />
```
