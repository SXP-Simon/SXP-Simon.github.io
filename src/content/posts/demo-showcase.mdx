...
无需引入笨重的静态字体包，构建时自动编译为轻量 SVG 矢量图标：
...
<div class="flex flex-wrap items-center gap-6 my-6 p-5 rounded-2xl border border-border bg-foreground/5 not-prose">
  ...
  <div class="flex items-center gap-2">
    <Icon name="lucide:palette" class="size-5 text-accent" />
  ...
  <div class="flex items-center gap-2">
    <Icon name="lucide:music" class="size-5 text-accent" />
  ...
  <div class="flex items-center gap-2">
    <Icon name="lucide:bookmark" class="size-5 text-accent" />
...
> [!NOTE]
> **提示说明（NOTE）**：用于提供背景信息、补充说明或上下文上下文。

> [!TIP]
> **最佳实践建议（TIP）**：推荐将常用组件放在 `src/components/mdx/` 目录下方便快速复用。

> [!IMPORTANT]
> **重要提醒（IMPORTANT）**：生产打包时会自动生成离线 Pagefind 全文检索索引。

> [!WARNING]
> **警告提示（WARNING）**：如果修改了 `astro-paper.config.ts` 中的站点域名，请同步确认 `CNAME` 文件。

> [!CAUTION]
> **高危警示（CAUTION）**：敏感 Token 与密钥切勿直接提交到公开仓库中。
...
点击下方图片即可全屏预览，支持平滑弹出放大、背景变暗聚焦、滚轮/捏合缩放以及按 `ESC` 键关闭：
...
欢迎在导航栏右上角切换 **浅色 / 深色主题** 或使用 **搜索功能** 体验本站的完整功能。
